...
function buildGradientVars(colors: string[]) {
  const vars: Record<string, string> = {};
  for (let i = 0; i < 7; i++) {
    const c = colors[Math.min(COLOR_MAP[i], colors.length - 1)] || colors[0] || '#c084fc';
...
const BorderGlow = forwardRef<HTMLDivElement, BorderGlowProps>(function BorderGlow(
  {
    children,
    className = '',
    edgeSensitivity = 30,
  ...
  const lightSurface = useMemo(() => isLightColor(backgroundColor), [backgroundColor]);
  ...
  return (
    ...
    >
      ...
      <div className={`border-glow-inner ${innerClassName}`.trim()} style={innerStyle}>
        {children}
      </div>
    </div>
  );
